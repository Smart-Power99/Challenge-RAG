...
## 二次更新
支持了 BM25 稀疏字频匹配引擎。
并行检索引擎：现在每当用户在基于 Apple 10-K 的前端界发送生僻且高精硬分词要求时：
FAISS 引擎（权重 0.5）去查找字面上不像但具有此概念语义属性的句子；
BM25Retriever 引擎（权重 0.5）去字典死磕所有严格包含字符片段的句子；
算法重排：利用集成的 EnsembleRetriever，通过倒数融合 (RRF) 合成为一张终极精准的结果名单并送回大模型
